...
[Request](./Request) and [Reply](./Reply) are the core Fastify objects.

`done` is the function to continue with the [lifecycle](./Lifecycle).
...
set in [`bodyLimit`](./Server#bodylimit) option.
...
_(Note that the default `customErrorHandler` always sends the error back to the
user)_.
...
**Notice:** client abort detection is not completely reliable. See: [`Detecting-When-Clients-Abort/`](../Guides/Detecting-When-Clients-Abort/)
...
_The error will be handled by [`Reply`](./Reply#errors)._
...
It is useful when [plugins](./Plugins) need a "shutdown" event, for example,
...
HTTP requests have been completed.
It is useful when [plugins](./Plugins) have set up some state attached
...
Triggered when a new route is registered. Listeners are passed a [`routeOptions`](./Routes/#routes-options)
...
[plugins guide](../Guides/Plugins-Guide/). If you pass a function, that
...
the [Request](./Request) instance. This way, your controllers can read it
...
(for more about it, see [TypeScript](./TypeScript) page), like:
...
consider creating a custom [Plugin](./Plugins) instead.
