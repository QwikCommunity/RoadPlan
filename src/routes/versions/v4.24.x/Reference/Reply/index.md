...
- `.context` - Deprecated, access the [Request's context](./Request) property.
- `.routeOptions` - Access the [Request's routeOptions](./Request) property.
...
hook specified in [`setNotFoundHandler`](./Server/#set-not-found-handler).
...
hook](./Hooks#onresponse) it will always return `0`.
...
_send_ can also handle streams by setting the `'Content-Type'` header to
...
[`setErrorHandler`](./Server#seterrorhandler) API.
...
Check out the [Reply Lifecycle](./Lifecycle/#reply-lifecycle)
for more information.

The not found errors generated by the router will use the
[`setNotFoundHandler`](./Server#setnotfoundhandler)
...
[`onSend` hooks](./Hooks#onsend)) must be one of the following types,
...
[Routes#async-await](./Routes/#async-await).
