...
your input data against a JSON schema](./Validation-and-Serialization).
...
[`setErrorHandler`](./Server#seterrorhandler), the custom error handler will
...
context](./Encapsulation). Error handlers are fully encapsulated, so a
...
visit [`Middleware`](./Middleware) for more info.
