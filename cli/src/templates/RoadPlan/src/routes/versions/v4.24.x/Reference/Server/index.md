...
The default body reader sends [`FST_ERR_CTP_BODY_TOO_LARGE`](./Errors#fst_err_ctp_body_too_large)
reply, if the size of the body exceeds this limit.
If [`preParsing` hook](./Hooks#preparsing) is provided, this limit is applied
...
[Prototype Poisoning](../Guides/Prototype-Poisoning/bout
...
[secure-json-parse](https://github.com/fastify/secure-json-parse). See
[Prototype Poisoning](../Guides/Prototype-Poisoning/) for more details about
prototype poisoning attacks.
...
+ `object`: a standard Pino [options
  object](https://github.com/pinojs/pino/blob/c77d8ec5ce/docs/API/#constructor).
...
request-id](./Logging/#logging-request-id) section.
...
[#schema-validator](./Validation-and-Serialization/#schema-validator) section.
...
arguments received are of type [RawRequest](./TypeScript#rawrequest) and
[RawReply](./TypeScript#rawreply) respectively.
...
run the [`'onClose'`](./Hooks/#on-close) hook.
...
check [here](./Decorators).

#### register
<a id="register"></a>

Fastify allows the user to extend its functionality with plugins. A plugin can
be a set of routes, a server decorator, or whatever, check [here](./Plugins).
...
The logger instance, check [here](./Logging).

#### version
<a id="version"></a>

Fastify version of the instance. Used for plugin support. See
[Plugins](./Plugins/#handle-the-scope) for information on how the version is
used by plugins.
...
[here](../Guides/Testing/#benefits-of-using-fastifyinject).
...
Serialization](./Validation-and-Serialization) documentation.
...
[Reply.serializer(func)](./Reply#serializerfunc) has not been set. The
...
[#schema-validator](./Validation-and-Serialization/#schema-validator).

#### setSchemaErrorFormatter
<a id="set-schema-error-formatter"></a>

Set the schema error formatter for all routes. See
[#error-handling](./Validation-and-Serialization#schemaerrorformatter).

#### setSerializerCompiler
<a id="set-serializer-resolver"></a>

Set the schema serializer compiler for all routes. See
[#schema-serializer](./Validation-and-Serialization/#schema-serializer).
  ...
  schemaController: {
    ...
    compilersFactory: {
      ...
      buildValidator: function factory (externalSchemas, ajvServerOption) {
        ...
        // See [#schema-validator](./Validation-and-Serialization/#schema-validator) for details.
      ...
      buildSerializer: function factory (externalSchemas, serializerOptsServerOption) {
        ...
        // See [#schema-serializer](./Validation-and-Serialization/#schema-serializer) for details.
...
handlers if a different [`prefix` option](./Plugins/#route-prefixing-option)
...
lifecycle](./Lifecycle#lifecycle). *async-await* is supported as well.

You can also register [`preValidation`](./Hooks/#route-hooks) and
[`preHandler`](./Hooks/#route-hooks) hooks for the 404 handler.
...
> calls [`reply.callNotFound`](./Reply/#call-not-found). In which case, only
